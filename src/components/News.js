  ...
  handleNextClick = async () => {
    if (this.state.page + 1 < Math.ceil(this.state.totalResults / 12)) {
      ...
      let url = `https://newsapi.org/v2/top-headlines?country=${
        ...
        this.props.category
      }&apiKey=8be6b0c0182d4b67981323e7de265289&page=${
        this.state.page + 1
      }&pageSize=12`;
      this.setState({ loading: true });
      let data = await fetch(url);
            ...
            ? this.state.articles.map((element) => {
                return (
                  <div className="col-md-4" key={element.url}>
                    <NewsItems
                      title={element.title ? element.title.slice(0.32) : ""}
                      description={
                        element.description
                      ...
                      imageUrl={
                        element.urlToImage
                          ? element.urlToImage
                          : "https://thumbs.dreamstime.com/b/news-icon-19655995.jpg"
                      }
                      timming={element.publishedAt}
